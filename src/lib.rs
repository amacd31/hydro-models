...
pub extern "C" fn gr4j_run(
    ...
    let precip = unsafe { std::slice::from_raw_parts(precip_ptr, n) };
    let pet = unsafe { std::slice::from_raw_parts(pet_ptr, n) };
    let mut qsim = unsafe { std::slice::from_raw_parts_mut(qsim_ptr, n) };
